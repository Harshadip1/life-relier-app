...
const STATUS_STYLE: Record<string, any> = {
  ...
  Collected: { bg: '#ECFDF5', color: T.green,    icon: 'check-circle-outline'   },
  ...
  return (
    <View style={[s.root, { paddingTop: Math.max(insets.top, 0) }]}>
      ...
      <View style={s.searchBar}>
        ...
        <TextInput style={s.searchInput} placeholder="Search by name or PID..." placeholderTextColor={T.muted} value={search} onChangeText={setSearch} />
          ...
          return (
            <View style={s.card}>
              ...
              <View style={s.detailRow}>
                <MaterialCommunityIcons name="clock-outline" size={13} color={T.sub} />
                <Text style={s.detailTxt}>  {item.time}</Text>
                <MaterialCommunityIcons name="test-tube" size={13} color={T.sub} style={{ marginLeft: 16 }} />
...
const s = StyleSheet.create({
  ...
  searchBar:   { flexDirection: 'row', alignItems: 'center', backgroundColor: T.card, margin: 16, borderRadius: 10, borderWidth: 1, borderColor: T.border, paddingHorizontal: 12, height: 44 },
  searchInput: { flex: 1, fontSize: 13, color: T.text },
  scanBtn:     { width: 32, height: 32, borderRadius: 8, backgroundColor: T.tealDark, alignItems: 'center', justifyContent: 'center' },
  list:        { paddingHorizontal: 16, paddingBottom: 100 },
  card:        { backgroundColor: T.card, borderRadius: 14, borderWidth: 1, borderColor: T.border, padding: 14 },
  cardHeader:  { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' },
  name:        { fontSize: 14, fontWeight: '700', color: T.text },
  pid:         { fontSize: 11, color: T.sub, marginTop: 2 },
  ...
  detailTxt:   { fontSize: 12, color: T.sub },
  ...
  collectBtn:  { flex: 2, height: 36, borderRadius: 8, backgroundColor: T.tealDark, flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
