...
export default function PaymentGatewayScreenEnhanced({ navigation, route }: any) {
  const { user } = useAuth();
  ...
  const invoiceNo = params.invoiceNumber || params.billNo || `PT${String(params.PatRegID).padStart(6, '0')}`;

  const [selectedMethod, setSelectedMethod] = useState<PaymentMethod>('UPI'); // Default to UPI
  ...
  const handlePayNow = async () => {
    ...
    try {
      ...
      const order = await createPaymentOrder({
        ...
        amount: paymentAmount, // Backend will validate this
        currency: 'INR',
      ...
      // Step 2: Launch Razorpay native checkout
  ...
  return (
    <SafeAreaView style={styles.root}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Pay Outstanding Invoice</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.securityInfo}>
          ...
          <Text style={styles.securityText}>
            🔒 Secure payment powered by Razorpay. Your payment details are encrypted and not stored on our servers.
          </Text>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  amountLabel: { fontSize: 16, fontWeight: '700', color: COLORS.textPrimary },
